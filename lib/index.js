...
class GraphOps {
  constructor(nodes) {
    /*
    Description: Creates the graph and adds weighted edges to the nodes in order,
    i.e., node 0 connects to node 1, 1 to 2, and so-on.

    Arguments:
    nodes: An array of node coordinate pairs, which are also arrays
    */
    this.cy = cytoscape({});
    ...
    for (i = 0; i < M - 1; i++) {
      ...
      var xd =
        this.cy.$('#' + target).position().x - this.cy.$('#' + source).position().x;
      var yd =
        this.cy.$('#' + target).position().y - this.cy.$('#' + source).position().y;
...
*/
    ...
    while (endwhile === false) {
      ...
      if (rem === true) {
        ...
        while (uniqedge === false) {
          ...
          source = '_node_' + Math.floor(Math.random() * numnodes);
          target = '_node_' + Math.floor(Math.random() * numnodes);
  ...
  */
    ...
    for (i = 0; i < this.cy.nodes().length; i++) {
      Adj[i] = [];
      for (j = 0; j < this.cy.nodes().length; j++) {
        Adj[i][j] = 0;
      }
    }
    ...
    */
    ...
    if (randomsource === true) {
      dijkstra = this.cy
        .elements()
        .dijkstra(
          '#_node_' + Math.floor(Math.random() * this.cy.nodes().length),
          function(edge) {
            return edge.data('weight');
          }
        );
    } else {
      dijkstra = this.cy.elements().dijkstra('#_node_0', function(edge) {
        return edge.data('weight');
      });
    }
...
*/
  ...
  for (i = 0; i < N; i++) {
    ...
    theta1 = graph.theta(graph.cy, r, true);
    ...
    theta2 = graph.theta(graph.cy, r, true);
...
Most frequent graphs are ordered such that the earlier ones show up first.
...
freq: the frequency of each graph at the same index in the graphs array
*/
...
function main(inputs) {
  /*
  Description: Executes the main function
  Arguments:
  inputs: The inputs for r, nodes, T, N, and topcent, in array form
  ...
  */
  ...
  // Get the most frequent graphs
  var adjacencysorted = frequentgraphs(adjacencymatrices, topcent);
  console.log('Expected maximum shortest path length is ' + expectmaxshort);
  console.log('Expected number of edges attached to node 0 is ' + expectsedges);
  console.log('Expected total number of edges is ' + expecttotedges);

  for (i = 0; i < adjacencysorted.length; i++) {
    console.log(adjacencysorted[0][i]);
    console.log('with a frequency of ' + adjacencysorted[1][i]);
  }
...
module.exports = { GraphOps, frequentgraphs, methastings, acrej, main };
