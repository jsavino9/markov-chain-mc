...
function frequentgraphs(array,topcent) {
  var frequency = {}, value=[[]];
  var hold = 0;
  for(var i = 0; i < array.length; i++) {
    value = array[i];
    if(value in frequency) {
      hold = frequency[value][0]+1;
      frequency[value] = [hold,value];
      //console.log(frequency);
    }
    else {
      frequency[value] = [1,value];
      //frequency[value] = value;
    }
  }
  console.log(frequency[value][0]);
  var uniques = [];
  var freq = [];
  for(value in frequency) {
    uniques.push(frequency[value]);
  }
  var sorted = uniques.sort(function compareFrequency(a, b) {
          return b[0] - a[0];
      });
  var graphs = [];
...
console.log("length is " + sorted.length)
      return graphs;
  }
